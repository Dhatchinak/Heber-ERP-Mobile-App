          ...
          Padding(
            ...
            child: Column(
              children: [
                _animated(
                    0,
                    _infoCard("ACADEMIC DETAILS", Icons.school_rounded, c.cyan,
                        _academicRows(c), c)),
                const SizedBox(height: 14),
                _animated(
                    1,
                    _infoCard("PERSONAL INFORMATION", Icons.person_rounded,
                        c.violet, _personalRows(c), c)),
                const SizedBox(height: 14),
                _animated(
                    2,
                    _infoCard("FAMILY DETAILS", Icons.family_restroom_rounded,
                        c.amber, _familyRows(c), c)),
                const SizedBox(height: 14),
                _animated(
                    3,
                    _infoCard(
                ...
                if (studentData?["academic_info"] != null &&
                  ...
                  _animated(
                      4,
                      _infoCard("PREVIOUS ACADEMIC", Icons.history_edu_rounded,
                          c.pink, _prevAcademicRows(c), c)),
                ],
                const SizedBox(height: 14),
                _animated(
                    5,
                    _infoCard("ADDITIONAL INFO", Icons.info_rounded, c.cyan,
                        _additionalRows(c), c)),
              ],
            ),
          ),
...
// ─── HERO HEADER ─────────────────────────────────────────────────────────
                  ...
                  children: [
                      ...
                      builder: (_, __) => Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: c.cyan
                                  .withOpacity(0.2 + _pulseCtrl.value * 0.12),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        border: Border.all(
                            color: c.cyan.withOpacity(0.6), width: 2.5),
                        color: c.elevated,
                        boxShadow: [
                          BoxShadow(
    ...
    return Container(
      ...
      child: Center(
        child: initials.isNotEmpty
            ? Text(initials,
                style: TextStyle(
                    color: c.violet, fontSize: 32, fontWeight: FontWeight.w900))
          ...
          if (icon != null) ...[
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: (color ?? c.textLow).withOpacity(0.2)),
              ),
  ...
  List<Widget> _academicRows(ThemeProvider c) => [
        _infoRow("Program Name",
            studentData?["current_academic"]?["program_name"] ?? "", c,
            icon: Icons.auto_stories_rounded, color: c.cyan),
        _infoRow("Department",
            studentData?["current_academic"]?["department_name"] ?? "", c,
            icon: Icons.account_balance_rounded, color: c.violet),
        _infoRow(
            "Section", studentData?["current_academic"]?["section"] ?? "", c,
            icon: Icons.group_work_rounded, color: c.green),
        _infoRow("Program Type",
            studentData?["current_academic"]?["program_type"] ?? "", c,
            icon: Icons.category_rounded, color: c.amber),
        _infoRow("Admission Number",
            studentData?["admission_number"]?.toString() ?? "", c,
            icon: Icons.confirmation_num_rounded, color: c.violet),
        _infoRow("Admission Date",
            studentData?["admission_date"]?.split("T")[0] ?? "", c,
            icon: Icons.calendar_today_rounded, color: c.cyan),
      ];

  List<Widget> _personalRows(ThemeProvider c) => [
        _infoRow("Date of Birth",
            studentData?["personal_info"]?["dob"]?.split("T")[0] ?? "", c,
            icon: Icons.cake_rounded, color: c.pink),
        _infoRow("Gender", studentData?["personal_info"]?["gender"] ?? "", c,
            icon: Icons.person_outline_rounded, color: c.violet),
        _infoRow("Blood Group",
            studentData?["personal_info"]?["blood_group"] ?? "", c,
            icon: Icons.bloodtype_rounded, color: c.pink),
        _infoRow(
            "Aadhar Number", studentData?["personal_info"]?["aadhar"] ?? "", c,
            icon: Icons.credit_card_rounded, color: c.cyan),
        _infoRow(
            "Community", studentData?["personal_info"]?["community"] ?? "", c,
            icon: Icons.people_rounded, color: c.amber),
        _infoRow("Mother Tongue",
            studentData?["personal_info"]?["mother_tongue"] ?? "", c,
            icon: Icons.language_rounded, color: c.green),
      ];

  List<Widget> _familyRows(ThemeProvider c) => [
        _infoRow(
            "Father's Name",
            studentData?["personal_info"]?["family"]?["father"]?["name"] ?? "",
            c,
            icon: Icons.person_2_rounded,
            color: c.cyan),
        _infoRow(
            "Father's Occupation",
            studentData?["personal_info"]?["family"]?["father"]
                    ?["occupation"] ??
                "",
            c,
        ...
        _infoRow(
            "Father's Mobile",
            studentData?["personal_info"]?["family"]?["father"]?["mobile_no"] ??
                "",
            c,
        ...
        _infoRow(
            "Mother's Name",
            studentData?["personal_info"]?["family"]?["mother"]?["name"] ?? "",
            c,
            icon: Icons.person_2_rounded,
            color: c.pink),
        _infoRow(
            "Mother's Occupation",
            studentData?["personal_info"]?["family"]?["mother"]
                    ?["occupation"] ??
                "",
            c,
            icon: Icons.work_rounded,
        ...
        _infoRow(
            "Mother's Mobile",
            studentData?["personal_info"]?["family"]?["mother"]?["mobile_no"] ??
                "",
            c,
            icon: Icons.phone_rounded,
            color: c.green),
  ...
  List<Widget> _contactRows(ThemeProvider c) => [
        _infoRow(
            "Email Address", studentData?["contact"]?["student_email"] ?? "", c,
            icon: Icons.email_rounded, color: c.pink),
        _infoRow("Mobile Number",
            studentData?["contact"]?["mobile_no"]?.toString() ?? "", c,
            icon: Icons.phone_iphone_rounded, color: c.green),
        _infoRow(
            "Current Address",
            _buildAddressString(
                studentData?["contact"]?["address"]?["communication"]),
            c,
  ...
  List<Widget> _prevAcademicRows(ThemeProvider c) {
    ...
    return [
      _infoRow("Qualification", info[0]["qualification"] ?? "", c,
          icon: Icons.book_rounded, color: c.violet),
      _infoRow("Institution", info[0]["institution"] ?? "", c,
          icon: Icons.account_balance_rounded, color: c.amber),
      _infoRow("Board/University", info[0]["board"] ?? "", c,
          icon: Icons.school_rounded, color: c.cyan),
      _infoRow("Percentage", info[0]["percentage"]?.toString() ?? "", c,
          icon: Icons.percent_rounded, color: c.green),
      _infoRow("UMIS Number", info[0]["umis"] ?? "", c,
          icon: Icons.numbers_rounded, color: c.pink),
    ];
  }

  List<Widget> _additionalRows(ThemeProvider c) => [
        _infoRow("Application Number",
            studentData?["application_no"]?.toString() ?? "", c,
            icon: Icons.numbers_rounded, color: c.cyan),
        _infoRow("Shift", studentData?["shift"] ?? "", c,
            icon: Icons.schedule_rounded, color: c.amber),
        _infoRow("Status", studentData?["status"] ?? "", c,
            icon: Icons.circle_rounded,
            color: studentData?["status"] == "Active" ? c.green : c.pink),
        _infoRow("Registration Date",
            studentData?["registration_date"]?.split("T")[0] ?? "", c,
            icon: Icons.date_range_rounded, color: c.violet),
      ];
      ...
      child: Column(
        ...
        children: [
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Icon(Icons.person_rounded,
                      size: 24, color: c.violet.withOpacity(0.6)),
                ],
              ),
            ),
          ),
          ...
          Text("Loading Profile...",
              style: TextStyle(
                  fontSize: 14, color: c.textMid, fontWeight: FontWeight.w600)),
